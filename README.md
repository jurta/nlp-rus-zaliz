...
Файл с раскрытыми формами словоизменения
...
Есть слова, имеющие несколько значений, где каждое значение может иметь
...
Список свойств разделяется по длине на имеющие короткие и длинные значения.
В сопоставлении с именем `с` записываются короткие свойства.
Пример, где в первой строке короткие значения, а во второй строке - длинное:

```yaml
аваль:
 с: {т: с, р: м, и: 2, у1: a}
 з: вексельное поручительство
```

Все возможные значения коротких свойств под именем `с`:
т т2 р рм о и у1 у2 р2 п2 п2ф ч чё чо ч2 ч3 гв гп гмн гбл гпс ос осф до фз фн фп

Возможные значения остальных длинных свойств, которые остаются на первом уровне:
с мн с2ч гпр гпрс сб фк фр слсч з з2 з3 о о1 о2 о3 о4 о5 п
...
где в словоформе `амѝнокислота́` используется основа `о`,
а в `амѝнокисло́ты` - основа `о1`.
