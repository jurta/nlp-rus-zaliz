...
while (<>) { # (sort (keys (%ADB_File::wordpos)))
  ...
  for $wihp (@wis) {
    ...
    $props{'Ó'} = "{" . join(", ", map ({
      "$_" . (($syntax_props{$_} ne ":") ? (($_ eq 'ÇÐÒ' && $syntax_props{$_} =~ /[\[:\]]/)? ': "'.($syntax_props{$_} =~ s/"//g, $syntax_props{$_}).'"' : ": ".($syntax_props{$_})) : ": t")
    } sort { $syntax_order{$a} <=> $syntax_order{$b} } (keys %syntax_props))) . "}";

    $props{'Ç'} = "{" . join(", ", map ({
      "$_" . (($grammar_props{$_} ne ":") ? ((($_ eq 'ÆÎ' || $_ eq 'ÆÚ') && $grammar_props{$_} =~ /[\[\]]/)? ': "'.$grammar_props{$_}.'"' : ": ".$grammar_props{$_}) : ": t")
    } sort { $grammar_order{$a} <=> $grammar_order{$b} } (keys %grammar_props))) . "}";

    print "$w:", map ({
      "\n $_" . (($props{$_} ne ":") ? ((($_ eq 'Ú' || $_ eq 'Ú3' || $_ eq 'ÆÒ') && $props{$_} =~ /:/) ? ': "'.($props{$_} =~ s/"//g, $props{$_}).'"': ": ".$props{$_}) : ": t")
